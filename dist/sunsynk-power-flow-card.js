...
//    }
    ...
    let solar_animation_speed = ""
    if (config && config.solar && config.solar.animation_speed) {
      let speed = config.solar.animation_speed * totalsolar / config.solar.max_power || totalsolar
      solar_animation_speed = `${speed}s` 
    } else {
      solar_animation_speed = "9s"
    }
    
    let battery_animation_speed = "6s"
    if (config && config.battery && config.battery.animation_speed) {
      let speed = config.battery.animation_speed * parseInt(stateObj13.state) / config.battery.max_power || parseInt(stateObj13.state)
      battery_animation_speed = `${speed}s` 
    } else {
      battery_animation_speed = "6s"
    }
    
    let load_animation_speed = "4s"
    if (config && config.load && config.load.animation_speed) {
      let speed = config.load.animation_speed * parseInt(stateObj14.state) / config.load.max_power || parseInt(stateObj14.state)
      load_animation_speed = `${speed}s` 
    } else {
      load_animation_speed = "4s"
    }
    
    let grid_animation_speed = "8s"
    if (config && config.grid && config.grid.animation_speed) {
      let speed = config.grid.animation_speed * parseInt(stateObj15.state) / config.grid.max_power || parseInt(stateObj15.state)
      grid_animation_speed = `${speed}s` 
    } else {
      grid_animation_speed = "8s"
    }
        ...
        <div class="container card">
          <svg viewBox="-0.5 -0.5 457 383" height="${panel === 'no' ? '396px' : '100%'}" width="100%" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1">
            ...
            <circle id="pv1-dot" cx="0" cy="0" r="3" class="${config.show_solar === 'no' ? 'st12' : ''}" fill="${parseInt(stateObj9.state) <= 0 ? 'transparent' : `${solar_colour}`}">
              <animateMotion dur="${solar_animation_speed}" repeatCount="indefinite" keyPoints="1;0" keyTimes="0;1" calcMode="linear">
            ...
            <circle id="power-dot-charge" cx="0" cy="0" r="3" fill="${parseInt(battery_power) < 0 || parseInt(battery_power) === 0 ? 'transparent' : `${battery_colour}`}">
              <animateMotion dur="${battery_animation_speed}" repeatCount="indefinite" keyPoints="1;0" keyTimes="0;1" calcMode="linear">
            ...
            <circle id="power-dot-discharge" cx="0" cy="0" r="3" fill="${parseInt(battery_power) > 0 || parseInt(battery_power) === 0 ? 'transparent' : `${battery_colour}`}">
              <animateMotion dur="${battery_animation_speed}" repeatCount="indefinite" keyPoints="0;1" keyTimes="0;1" calcMode="linear">
                <mpath xlink:href="#bat-line"/>
              </animateMotion>
            </circle>
            ...
            <circle id="grid-dot" cx="0" cy="0" r="3" fill="${parseInt(stateObj15.state) < 0 || parseInt(stateObj15.state) === 0 ? 'transparent' : `${grid_colour}`}">
              <animateMotion dur="${grid_animation_speed}" repeatCount="indefinite" keyPoints="1;0" keyTimes="0;1" calcMode="linear">
                <mpath xlink:href="#grid-line"/>
              </animateMotion>
            </circle>
            <circle id="grid-dot" cx="0" cy="0" r="3" fill="${parseInt(stateObj15.state) > 0 || parseInt(stateObj15.state) === 0 ? 'transparent' : `${grid_colour}`}">
              <animateMotion dur="${grid_animation_speed}" repeatCount="indefinite" keyPoints="0;1" keyTimes="0;1" calcMode="linear">
                <mpath xlink:href="#grid-line"/>
              </animateMotion>
            </circle>
            ...
            <circle id="aux-dot" cx="0" cy="0" r="3" class="${show_aux === 'no' || aux_power === 0 ? 'st12' : ''}" fill="${parseInt(aux_power) > 0  ? 'transparent' : `${load_colour}`}">
              <animateMotion dur="${load_animation_speed}" repeatCount="indefinite" keyPoints="1;0" keyTimes="0;1" calcMode="linear">
                <mpath xlink:href="#aux-line"/>
              </animateMotion>
            </circle>
            ...
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.ac_temp_91)}>
              <text id="ac_temp" x="34%" y="65%" class="st10 left-align" display="${inverter_show_temp === 'no' ? 'none' : ''}" fill="white" >${parseInt(stateObj37.state) ? parseFloat(stateObj37.state) : '0'} C</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.dc_temp_90)}>
              <text id="dc_temp" x="41%" y="65%" class="st10 left-align" display="${inverter_show_temp === 'no' ? 'none' : ''}" fill="white" >${parseInt(stateObj38.state) ? parseFloat(stateObj38.state) : '0'} C</text>
            </a>
