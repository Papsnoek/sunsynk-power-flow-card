...
class SunsynkPowerFlowCard extends LitElement {
  ...
  static getStubConfig() {
    return {
      ...
      entities: {
        ...
        ac_temp_91: 'sensor.ac_temp',
        dc_temp_90: 'sensor.dc_temp',
      }
    };
  }
  ...
  render() {
    ...
    const stateObj37 = this.hass.states[config.entities.ac_temp_91] || { state: '0' };
    const stateObj38 = this.hass.states[config.entities.dc_temp_90] || { state: '0' };
...
//    }
    ...
    let inverter_show_temp = '';
    if (config && config.inverter && config.inverter.show_temp) {
      inverter_show_temp = config.inverter.show_temp; // override with provided value
    } else {
      inverter_show_temp = 'no'; // set default value
    }
    ...
    if (config && config.battery && config.battery.animation_speed) {
      let speed = config.battery.animation_speed - ((config.battery.animation_speed-1) * (parseInt(stateObj13.state) / config.battery.max_power || totalsolar))
    ...
    if (config && config.load && config.load.animation_speed) {
      let speed = config.load.animation_speed - ((config.load.animation_speed-1) * (parseInt(stateObj14.state) / config.load.max_power || totalsolar))
    ...
    if (config && config.load && config.load.animation_speed) {
      let speed = config.load.animation_speed - ((config.load.animation_speed-1) * (parseInt(stateObj24.state) / config.load.max_power || totalsolar))
    ...
    if (config && config.grid && config.grid.animation_speed) {
      let speed = config.grid.animation_speed - ((config.grid.animation_speed-1) * (parseInt(stateObj15.state) / config.grid.max_power || totalsolar))
    ...
    if (config && config.load && config.load.animation_speed) {
      let speed = config.load.animation_speed - ((config.load.animation_speed-1) * (parseInt(stateObj34.state) / config.load.max_power || totalsolar))
      ...
      return html`
        <div class="container card">
          <svg viewBox="-0.5 -0.5 457 383" height="${panel === 'no' ? '396px' : '100%'}" width="100%" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1">
            ...
            <rect x="145.15" y="${inverter_show_temp==='no' ? '162' : '153'}" width="70" height="${inverter_show_temp==='no' ? '50' : '70'}" rx="10.5" ry="10.5" fill="none" stroke="${inverter_colour}" pointer-events="all"/>
            ...
            <path id="aux-line2" d="M 200 ${inverter_show_temp==='no' ? '162' : '152'} L 200 57 Q 200 47 210 47 L 237 47" fill="none" class="${show_aux === 'no' ? 'st12' : ''}" stroke="${load_colour}" stroke-width="1" stroke-miterlimit="10"  pointer-events="stroke"/> 
            ...
            <path d="M 180.15 ${inverter_show_temp==='no' ? '212' : '223'} L 180.15 235" fill="none" stroke="${inverter_colour}" stroke-width="1" stroke-miterlimit="10"  pointer-events="stroke"/>
            ...
            <path id="es-line" d="M 235 118 L 212 118 Q 200 118 200 128 L 200 ${inverter_show_temp==='no' ? '162' : '152'}" fill="none" stroke="${load_colour}" stroke-width="1" stroke-miterlimit="10"  pointer-events="stroke"/>  
            ...
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.inverter_out_175)}>
              <text id="inverter_out_175" x="39.5%" y="${inverter_show_temp==='no' ? '46.5%' : '44%'}" class="${font === 'no' ? 'st14' : 'st4'} st8" fill="${inverter_colour}">${stateObj22.state ? stateObj22.state : '0'} W</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.inverter_out_164)}>
              <text id="inverter_out_164" x="39.5%" y="${inverter_show_temp==='no' ? '52%' : '48%'}" class="${font === 'no' ? 'st14' : 'st4'} st8" fill="${inverter_colour}">${stateObj7.state ? stateObj7.state : '0'} A</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.ac_temp_91)} display="${inverter_show_temp === 'no' ? 'none' : ''}">
            ...
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.dc_temp_90)} display="${inverter_show_temp === 'no' ? 'none' : ''}">
